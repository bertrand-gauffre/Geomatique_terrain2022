...
buffer_parcellesCible<-st_buffer(center_parcellesCible, 1000, 50) 
...
aires_buffer_parcellesCible<-inter_VergerFilet_buffer_parcellesCible %>% 
  ...
  mutate(percent_verger_filet=as.numeric(area_filets/(3.141016*1000*1000))*100) ## buffer 500
...
aires_buffer_parcellesCible_bio<-inter_buffer_parcellesCible_bio %>% 
  ...
  mutate(percent_verger_bio=as.numeric(area_AB/(3.141016*1000*1000))*100)
...
compo_VEGET_buffer_parcellesCible <- areas_VEGET_buffer_parcellesCible %>% 
  ...
  mutate(FID_plot=as.character(FID_plot))%>%
  mutate(area_tot_buffer=(3.141016*1000*1000))

#### ATTZENTION VERIFIER QUE cA CORRESPOND AVANT
colnames(compo_VEGET_buffer_parcellesCible) <- c("FID_plot", "bois", "foret_fermee_feuillus", "haie", "verger", "vigne", "lande_ligneuse", "foret_fermee_coniferes", "foret_fermee_mixte", "foret_ouverte", "area_tot_buffer")
...
plot<-ggplot(vergers_EXCLU_paysage, aes(x=percent_of_orchards_with_nets, y=percent_haie, size=as.factor(exclu_all), shape=filet_ok, alpha=as.factor(exclu_all), color=as.factor(exclu_all))) + 
  geom_point() +
  xlab("% vergers qui ont des filets")+
  ylab("% haies ")
...
## Faire une figure
p <- vergers_exclu %>%
  ggplot( aes(x=percent_of_orchards_with_nets)) +
  geom_histogram( binwidth=5, fill="#69b3a2", color="#e9ecef", alpha=0.9) +
