...
buffer_parcellesCible<-st_buffer(center_parcellesCible, 250, 50) 
...
aires_buffer_parcellesCible<-inter_VergerFilet_buffer_parcellesCible %>% 
  ...
  mutate(percent_verger_filet=as.numeric(area_filets/(3.141016*250*250))*100) ## buffer 500
...
aires_buffer_parcellesCible_bio<-inter_buffer_parcellesCible_bio %>% 
  ...
  mutate(percent_verger_bio=as.numeric(area_AB/(3.141016*250*250))*100)
...
compo_VEGET_buffer_parcellesCible <- areas_VEGET_buffer_parcellesCible %>% 
  ...
  mutate(FID_plot=as.character(FID_plot))%>%
  mutate(area_tot_buffer=(3.141016*250*250))

#### ATTZENTION VERIFIER QUE cA CORRESPOND AVANT
colnames(compo_VEGET_buffer_parcellesCible) <- c("FID_plot", "foret_fermee_feuillus", "haie", "verger", "bois", "vigne", "lande_ligneuse", "foret_ouverte","foret_fermee_mixte", "foret_fermee_coniferes", "area_tot_buffer")
...
################################################################################
candidats_ABnoFilet
candidats_ABnoFilet$candidat<-1
...
#### FIGURE AVEC DOUBLE GRADIENT  % vergers avec filets vs ESN
plot<-ggplot(vergers_EXCLU_paysage, aes(x=percent_of_orchards_with_nets, y=percent_ESN, size=as.factor(exclu_all), shape=filet_ok, alpha=as.factor(exclu_all), color=as.factor(exclu_all))) + 
  geom_point() +
  xlab("% vergers qui ont des filets")+
  ylab("% ESN ")
...
## juste les exclu
vergers_exclu<-vergers_candidats %>% filter(exclu=="oui" & valide_2022=="ok")
cor.test(vergers_exclu$percent_ESN,vergers_exclu$percent_of_orchards_with_nets, method = "pearson")
